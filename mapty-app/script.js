...
class App {
  ...
  _loadMap(position) {
    ...
    // Rendering Markers on the map (here, because at THIS POINT, the 'map' is already Loaded.)
  ...
  _moveToPopup(e) {
    ...
    //// DISABLED IT BECAUSE OF LOCAL STORAGE, READ NOTE !
  ...
  //// Using Local Storage API that the Browser provides (advised to use for Small Amounts of Data, because Local Storage is BLOCKING)
  // Local storage is a simple Key Value Store, and I need a simple Value, which must also be a String
  // 1st Argument 'name', 2nd Argument a String that I want to store, and will be associated with this 'workouts' Key
  // so for 2nd Argument I'm converting an Object to a String with JSON.stringify()

  ////// NOTE: WHEN CONVERTING THE OBJECTS TO A STRING, AND THEN BACK FROM THE STRING TO OBJECTS (USING JSON.stringify() & JSON.parse() ), THE PROTOTYPE CHAIN WILL BE GONE!!!
  ////// SO THESE NEW OBJECT THAT WERE RECOVERED FROM LOCAL STORAGE ARE NOW JUST REGULAR OBJECTS. THEY ARE NOW NO LONGER OBJECTS THAT WERE CREATED BY THE Running or the Cycling CLASS.
  ////// THEREFORE THEY WILL NOT BE ABLE TO INHERIT ANY OF THEIR METHODS. (THIS COULD BE A BIG PROBLEM WHEN WORKING WITH LOCAL STORAGE AND OOP)

  _setLocalStorage() {
    localStorage.setItem('workouts', JSON.stringify(this.#workouts)); // setting all the workouts to Local Storage
  }

  _getLocalStorage() {
    // getting the localStorage and converting the String back to the Object using JSON.parse()
    const data = JSON.parse(localStorage.getItem('workouts'));
    console.log(data);

    // Checking if there is some Data
    // It these is Nothing for e.g. Undefined, then I don't want to do anything
    if (!data) return;
    // I want the Data from (Array of 'workouts') to be Restored
    this.#workouts = data; // data that was just read
    // Taking All of these 'workouts' and render them in the List
    // looping over the Array of 'workouts' WITHOUT creating a New Array
    // This Method will Render the workouts in the UI sidebar
    this.#workouts.forEach(work => {
      this._renderWorkout(work); // (work) the Current Workout
    });
  }

  // Adding reset() to the Public Interface to App Class
  // For deleting Local Storage, going to do it in Console, typing in app.reset()
  reset() {
    localStorage.removeItem('workouts');
    // Reloading the Page programmatically (the Page will look completely Empty)
    location.reload();
  }
}
